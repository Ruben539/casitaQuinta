...
$alert = '';
	...
	//exit; sirve para ejectuar la consulta en mysql
		...
		if ($resultado > 0) {
			...
			$sql_update = mysqli_query($conection,"UPDATE servicios SET descripcion = '$descripcion',precio = '$precio'
				WHERE id = $id");
...
$sql = mysqli_query($conection,"SELECT * FROM servicios  WHERE id = $id AND estatus = 1");   
